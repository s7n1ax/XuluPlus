...
extends Module {
    ...
    public void onTickEvent(TickEvent.ClientTickEvent event) {
        if (PortalViewerYippie.mc.field_71441_e == null) {
            return;
        }
    ...
    private void compileDL() {
        if (PortalViewerYippie.mc.field_71441_e == null || PortalViewerYippie.mc.field_71439_g == null) {
            return;
        }
        for (int x = (int)PortalViewerYippie.mc.field_71439_g.field_70165_t - this.distance.getValue(); x <= (int)PortalViewerYippie.mc.field_71439_g.field_70165_t + this.distance.getValue(); ++x) {
            for (int y = (int)PortalViewerYippie.mc.field_71439_g.field_70163_u - this.distance.getValue(); y <= (int)PortalViewerYippie.mc.field_71439_g.field_70163_u + this.distance.getValue(); ++y) {
                int z = (int)Math.max(PortalViewerYippie.mc.field_71439_g.field_70161_v - (double)this.distance.getValue().intValue(), 0.0);
                while ((double)z <= Math.min(PortalViewerYippie.mc.field_71439_g.field_70161_v + (double)this.distance.getValue().intValue(), 255.0)) {
                    BlockPos pos = new BlockPos(x, y, z);
                    Block block = PortalViewerYippie.mc.field_71441_e.func_180495_p(pos).func_177230_c();

...
extends Module {
    ...
    public Setting<Boolean> animateScale = this.register(new Setting<Boolean>("Scaling", false));
    ...
    public Setting<Integer> alpha = this.register(new Setting<Integer>("Saturation", 255, 0, 255));
    ...
    public Setting<Integer> hiddenAlpha = this.register(new Setting<Object>("Hidden Saturation", Integer.valueOf(255), Integer.valueOf(0), Integer.valueOf(255), v -> this.xqz.getValue() != false && this.rainbow.getValue() == false));
    public Map<EntityEnderCrystal, Float> scaleMap = new ConcurrentHashMap<EntityEnderCrystal, Float>();

    public Chams() {
        super("CrystalESP", "shows u an option for crystals rendering", Module.Category.RENDER, true, false, false);
        INSTANCE = this;
    }

    @Override
    public void onUpdate() {
        for (Entity crystal : Chams.mc.field_71441_e.field_72996_f) {
    ...
    public void onReceivePacket(PacketEvent.Receive event) {
        if (event.getPacket() instanceof SPacketDestroyEntities) {
            ...
            for (int id : packet.func_149098_c()) {
                Entity entity = Chams.mc.field_71441_e.func_73045_a(id);
    ...
    public void onRenderModel(RenderEntityModelEvent event) {
        ...
        boolean fancyGraphics = Chams.mc.field_71474_y.field_74347_j;
        Chams.mc.field_71474_y.field_74347_j = false;
        float gamma = Chams.mc.field_71474_y.field_74333_Y;
        Chams.mc.field_71474_y.field_74333_Y = 10000.0f;
        GL11.glPushMatrix();
